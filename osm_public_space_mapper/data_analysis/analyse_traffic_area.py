            ...
            def set_defaults(highway_default_widths:dict[str,tuple[float,float]], cycleway_default_widths:dict[dict[str:float]]) -> set[dict,dict]:
                if highway_default_widths is None:
                    highway_default_widths = {'footway': (1.8,1), 'service':(4.5, 3), 'residential':(4.5,3), 'steps':(2,1.5), 'tertiary':(4.8,3.1), 'primary':(5.5,3.1), 
                                            'cycleway':(2,1.5), 'secondary':(4.8,3.1), 'path':(1.5,1), 'motorway_link':(6.5,3.23), 'platform':(2,1.5), 'pedestrian':(2,2), 'motorway':(6.5,3.25), 'living_street':(4.5,3),
                                            'unclassified':(4.5,3), 'primary_link':(5.5,3.1), 'track':(3,2.5), 'corridor':(2,1), 'proposed':(4.8,3.1), 'secondary_link':(4.8,3.1), 'construction':(5.5,3.1), 'everything else':(4.8,3.1)}
            ...
            def estimate_road_width(element:OsmElement, highway_default_widths:dict[str,tuple[float,float]], cycleway_default_widths:dict[dict[str:float]]) -> float:
                ...
                def add_parking(element:OsmElement, 
                    ...
                    Args:
                        ...
                        default_parking_width (float, optional): _description_. Defaults to 6.5, assuming one side horizontal (2m) and one side angle parking (4.5m), taken from OSM Verkehrswende project https://parkraum.osm-verkehrswende.org/project-prototype-neukoelln/report/#27-fl%C3%A4chenverbrauch
